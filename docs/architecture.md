...
- `nudge/state.py` 管理本机 SQLite：计划、动作、健康汇总、习惯、agent 幂等记录、daemon 队列和运行记录。
...
flowchart TD
    ...
    approve --> idem[request_id 幂等]
    idem --> apple[Apple adapters]
    idem --> sqlite[SQLite agent_request_runs / actions]
    agent --> queue[daemon queue 可选异步执行]
    queue --> idem
```
...
- 真实写入必须带调用方生成的 `request_id`。同一 `request_id` + 同一 payload 重试会返回已存结果；同一 `request_id` + 不同 payload 会被拒绝，避免重复写 Apple 或隐藏变更。
...
- `command_queue`、`daemon_runs`：daemon 队列与执行记录。
- `agent_request_runs`：agent/MCP 幂等请求记录。
...
- 幂等：真实结构化写入要求 `request_id`，避免 retry 产生重复 Apple 项。
