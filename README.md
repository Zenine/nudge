...
| Reminders and completion tracking | `nudge reminders sync-completed`, `nudge reminders backfill-ids`, `nudge reminders backfill-lists`, `nudge log`, `nudge check-in`, `nudge feedback interview` | 同步一个或多个 Reminders 列表，回填外部 ID 或旧 action 的列表归属，并记录执行结果。`backfill-lists` 默认只读 `[reminders].sync_lists` 中的列表；重复 `--list` 可覆盖该范围。 | SQLite；`backfill-ids --apply` 会写 Reminders，`backfill-lists --apply` 只写本地 SQLite、不会改 Apple 数据 |
