...
def test_query_failure_keeps_diagnostics_but_disables_apply(monkeypatch) -> None:
    ...
    def fake_query(list_name, target_date):
        ...
        if list_name == "Tasks":
            return True, [{"name": "Buy milk", "due_at": "2026-07-01 09:00", "list": "Tasks"}]
        return True, []
    ...
    assert payload["errors"][0]["message"] == "private Apple adapter failure"
...
def test_sqlite_read_failure_has_stable_safe_error_and_zero_writes(monkeypatch) -> None:
    ...
    monkeypatch.setattr(command, "get_actions", fail_read)
...
def test_each_unique_list_date_is_queried_once_and_rows_receive_unique_keys(monkeypatch) -> None:
    ...
    def fake_query(list_name, target_date):
        ...
        return True, [
            {"name": "Buy milk", "due_at": "2026-07-01 09:00", "list": list_name},
            {"name": "Buy milk", "due_at": "2026-07-01 09:00", "list": list_name},
        ]
...
@pytest.mark.parametrize(
    ...
    [
        ["not an object"],
        [{"name": "Buy milk", "due_at": "2026-07-01 09:00", "list": "Other"}],
    ],
)
